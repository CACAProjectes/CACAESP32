      ...
      if (client.available()) {     // if there's bytes to read from the client,
        ...
        if (c == '\n') {            // if the byte is a newline character
          ...
          if (currentLine.length() > 0) {
			...
			// Reiniciar de cero la pagina_web
...
void gestionarPeticiones(String pCurrentLine) {
  ...
  Serial.print("iPos: ");
  Serial.print(String(iPos));
  Serial.print(" - ");
  Serial.println(pCurrentLine.substring(iPos+4, iPos+4+1));    
    if (iPos >= 0) {
...
String getPaginaWeb() {
  ...
  strPaginaWeb += String("<tr><td colspan=\"2\" align=\"center\"><h2>C A R</h2></td></tr>");
